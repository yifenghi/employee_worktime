package com.doodod.staffmanagement.data;
...
public class shopPolygon {
			...
			for (int j = 0; j < featureArr.length(); j++) {
				...
				if (layerType.equalsIgnoreCase("shop")) {
					...
					if("null".equalsIgnoreCase(""+properties.get("show_name"))){
						...
						if (!publicAreaName.containsKey(properties
							...
							sb.append(
									...
									Common.CTRL_A).append(publicAreaName.get(properties
											.get("category_name"))).append(
													Common.CTRL_A);
						}
					...
					try {
						...
						if (!publicAreaName.containsKey(properties
								.get("category_id"))) {
							sb.append("null").append(Common.CTRL_A).append(publicAreaName.get(properties
									.get("category_name"))).append(
											Common.CTRL_A);
						} else {
							sb.append(
				...
				sb.append(properties.get("category_id")).append(Common.CTRL_A).append(properties
						.get("category_name")).append(
								Common.CTRL_A);
	...
	public static void main(String[] args) throws Exception {
		...
		int mallId = 123;// Common.DEFAULT_MALL_ID;
		String path = "E:\\workspace\\employee_worktime\\data";
		...
		String publicAreaNamePath="E:\\workspace\\employee_worktime\\data\\publicArea_list";
